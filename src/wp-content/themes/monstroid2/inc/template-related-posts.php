...
function monstroid2_related_posts() {
	...
	$visible = monstroid2_theme()->customizer->get_value( 'related_posts_visible' );
	...
	$post_terms  = array();
	$post_number = monstroid2_theme()->customizer->get_value( 'related_posts_count' );
	...
	foreach ( $settings as $setting_key => $setting_value ) {
		$settings[ $setting_key ] = monstroid2_theme()->customizer->get_value( $setting_value );
	}
	...
	if ( $holder_view_dir ) {
		...
		foreach ( $posts as $post ) {
			...
			$image = ( $settings['image_visible'] ) ? monstroid2_post_thumbnail( 'monstroid2-thumb-s', array( 'echo' => false ) ) : '';
			...
			$author = ( $settings['author_visible'] ) ? monstroid2_posted_by( array( 'echo' => false ) ) : '';

			$date = ( $settings['date_visible'] ) ? monstroid2_posted_on( array( 'echo' => false ) ) : '';
